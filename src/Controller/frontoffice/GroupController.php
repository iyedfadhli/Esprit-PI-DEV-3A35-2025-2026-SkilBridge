...
public function index(EntityManagerInterface $em, Request $request): Response
{
    ...
    if ($quickPostForm->isSubmitted() && $quickPostForm->isValid()) {
        if (!$user) {
            $this->addFlash('error', 'You must be logged in to publish a post.');
...
public function show(Group $group, Request $request, EntityManagerInterface $em): Response
{
    ...
    {
        // Remove memberships linked to this group
        foreach ($em->getRepository(Membership::class)->findBy(['group_id' => $group]) as $m) {
            $em->remove($m);
        }
        // Remove posts and their dependencies linked to this group
        $posts = $em->getRepository(Posts::class)->findBy(['group_id' => $group]);
        foreach ($posts as $post) {
            foreach ($em->getRepository(Reactions::class)->findBy(['post_id' => $post]) as $r) {
                $em->remove($r);
            }
            foreach ($em->getRepository(Commentaires::class)->findBy(['post' => $post]) as $c) {
                $em->remove($c);
            }
            $em->remove($post);
        }
